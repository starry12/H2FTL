...
class RWCache_hot(object):
    ...
    def read(self, lpn, overall_cache_capacity): # rw_cache.read(ext.lpn_start)
        ...
        if lpn not in self.cache:
            ...
            writeback, wb_ppn = self.evict(overall_cache_capacity, state_preference="C")
            hit = False
        else: 
    ...
    def write(self, lpn, overall_cache_capacity): # hotness == hot
    ...
    def evict(self, overall_cache_capacity, state_preference): # hot
        writeback, wb_ppn = False, None
        ...
        if self.capacity + len(self.evict_hot) >= overall_cache_capacity:
            while self.capacity + len(self.evict_hot) >= overall_cache_capacity:
...
class RWCache_cold(object):
    ...
    def write(self, lpn, overall_cache_capacity, hotness):
        hit, writeback, wb_ppn = False, False, None
        if lpn not in self.cache:
            self.counter["write misses"] += 1
            self.cache._write(lpn, state="CL") 
            self.unassigned_cold._write(lpn)
            writeback, wb_ppn = self.evict(overall_cache_capacity, state_preference="A")
        else: # in cache
            hit = True
            self.counter["write hits"] += 1
            state = self.cache._read(lpn)
    ...
    def evict(self, overall_cache_capacity, state_preference): # cold
        writeback, wb_ppn = False, None
        if self.should_evict():
            lpn, state = self.cache.peak()
            if state == "A": 
        ...
        if self.capacity + len(self.evict_cold) >= overall_cache_capacity:
            while self.capacity + len(self.evict_cold) >= overall_cache_capacity:
                self.evict_cold.evict_one()
