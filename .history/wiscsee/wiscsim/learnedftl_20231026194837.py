...
class Ftl(ftlbuilder.FtlBuilder): # dftl，sftl，leaftl的初始化均使用这个类
    ...
    def __init__(self, confobj, recorderobj, flashobj, simpy_env, des_flash, ncq):
        ...
        self.rw_cache_hot = RWCache_hot(24*MB, self.conf.page_size, 8*MB, 1.0)
        self.rw_cache_cold = RWCache_cold(102*MB, self.conf.page_size, 8*MB, 1.0)
    ...
    def display_msg(self, mode):
        ...
        if self.hot_hit > self.cold_hit & self.rw_cache_cold.capacity >= (40 * MB) / (4 * KB): # cache lower
            print("data cache cold--")
            self.rw_cache_cold.change_capacity(False, 4 * MB)
            self.rw_cache_hot.change_capacity(True, 4 * MB)
        else:
            if self.rw_cache_hot.capacity >= (40 * MB) / (4 * KB):
        ...
        # ghost命中越多，需要空间越多
        if self.pb_mapping_table_hit > self.metadata.mapping_table.leaftl_hit & self.metadata.mapping_table.leaftl_size >= 40 * MB: # mapping table lower
            print("leaftl -- ")
            ...
            self.metadata.pb_mapping_table_size += 4 * MB
        ...
        #if self.hot_hit + self.cold_hit > self.pb_mapping_table_hit + self.metadata.mapping_table.leaftl_hit: # upper
            #if self.metadata.mapping_table.leaftl_size >= 40 * MB:
                ...
                #self.metadata.mapping_table.leaftl_size -= 4 * MB
                    ...
                    #self.metadata.pb_mapping_table_size -= 4 * MB
        #else:
            #if self.rw_cache_cold.capacity >= (40 * MB) / (4 * KB):
        ...
        print("hot data cache size", self.rw_cache_hot.capacity)
        print("cold data cache size", self.rw_cache_cold.capacity)
    ...
    def read_ext(self, extent): #读操作
        ...
        for ext in extents: # 遍历所有lpn
            ...
            if not cachehit: # 不在cold cache
                ...
                if evict_cold_hit:
                    self.cold_hit += 1
                cachehit, writeback, ppn = self.rw_cache_hot.read(ext.lpn_start)
            #else:
    ...
    def write_ext(self, extent, data=None): #写操作
        ...
        for i, ext in enumerate(extents): #遍历每个写入的lpn
            ...
            if int(ext.lpn_start)/(25*KB) not in self.HOTNESS:
                hit, writeback, ppn = self.rw_cache_cold.write(ext.lpn_start, hotness = 'cold')
                if hit == False:
                    self.rw_cache_hot.delete(ext.lpn_start)
            else:
                if self.HOTNESS[int(ext.lpn_start)/(25*KB)] == 'cold':
                    hit, writeback, ppn = self.rw_cache_cold.write(ext.lpn_start, hotness = 'cold') # 写入data cache / data buffer
                    if hit == False:
                        self.rw_cache_hot.delete(ext.lpn_start)
                    ...
                    hit, writeback, ppn = self.rw_cache_hot.write(ext.lpn_start, hotness = 'hot')
                    if hit == False:
    ...
    def clean(self, forced=False, merge=True): # gc --- 有效页面数在90%以下时擦除所有有效页面小于10%的block
        ...
        for lpn in validate_pages: #遍历有效页面
            if self.HOTNESS[int(lpn)/(25*KB)] == 'cold':
                hit, writeback, ppn = self.rw_cache_cold.write(lpn, hotness = 'cold') # 写入data cache / data buffer
                if hit == False:
                    self.rw_cache_hot.delete(lpn)
            else:
                hit, writeback, ppn = self.rw_cache_hot.write(lpn, hotness = 'hot')
                if hit == False:
...
class FlashMetadata(object):
    def __init__(self, confobj, counter): # 疑问： 没有Global Mapping Directory (GMD)? --- 如果不在memory里cache，如何知道对应的存储mapping table的物理位置?  --- gtd
        ...
        self.pb_mapping_table_size = 24 * 1024 * 1024
...
class FrameLogPLR:
    ...
    def __init__(self, conf, metadata, counter, gamma, max_size=1*1024**2, frame_length=256):
        ...
        self.leaftl_hit = 0
        self.leaftl_size = 150*MB # 584056832 bytes = 557 MB
