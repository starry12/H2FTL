...
class Ftl(ftlbuilder.FtlBuilder): # dftl，sftl，leaftl的初始化均使用这个类
    ...
    def __init__(self, confobj, recorderobj, flashobj, simpy_env, des_flash, ncq):
        ...
        self.written_bytes_2 = 0
        #self.discarded_bytes = 0
        self.pre_written_bytes_gc = 0
        self.read_bytes = 0
        self.read_bytes_2 = 0
        self.pre_written_bytes = 0
        self.pre_written_bytes_2 = 0
        #self.pre_discarded_bytes = 0
        self.pre_read_bytes = 0
        self.pre_read_bytes_2 = 0
        self.display_interval = 1000 * MB
        self.change_interval = 100 * MB
        #self.compaction_interval = 100 * MB
        #self.promotion_interval = 500 * MB
        self.gc_interval = 1 * MB
        self.rw_events = 0
        self.cache_average_size = 75*MB
        self.min_cache_size = 50*MB
        self.change_cache_size = 4*MB

        self.rw_cache_hot = RWCache_hot(self.cache_average_size, self.conf.page_size, 8*MB, 1.0)
        self.rw_cache_cold = RWCache_cold(self.cache_average_size, self.conf.page_size, 8*MB, 1.0)
    ...
    def auto_change_size(self):
        #print("hot hit", self.hot_hit)
        #print("cold hit", self.cold_hit)

        if self.auto_change == True:
            if self.hot_hit > self.cold_hit:  # cache lower
                if self.rw_cache_cold.capacity >= self.min_cache_size / self.change_cache_size:
                    print("data cache cold--")
                    self.rw_cache_cold.change_capacity(False, self.change_cache_size)
                    self.rw_cache_hot.change_capacity(True, self.change_cache_size)
            else:
                if self.rw_cache_hot.capacity >= self.min_cache_size / self.change_cache_size:
                    print("data cache hot--")
                    self.rw_cache_cold.change_capacity(True, self.change_cache_size)
                    self.rw_cache_hot.change_capacity(False, self.change_cache_size)
        

            # ghost命中越多，需要空间越多
            if self.pb_mapping_table_hit > self.metadata.mapping_table.leaftl_hit & self.metadata.mapping_table.leaftl_size >= self.min_cache_size: # mapping table lower
                print("leaftl -- ")
                self.metadata.mapping_table.leaftl_size -= self.change_cache_size
                self.metadata.pb_mapping_table_size += self.change_cache_size
            else:
                if self.metadata.pb_mapping_table_size >= self.min_cache_size:
            ...
            if (self.hot_hit + self.cold_hit) * 2 > self.pb_mapping_table_hit + self.metadata.mapping_table.leaftl_hit: # upper
                if self.metadata.mapping_table.leaftl_size >= self.min_cache_size:
            ...
            #print(self.metadata.pb_mapping_table_size/1024/1024, self.metadata.mapping_table.leaftl_size/1024/1024, self.rw_cache_hot.capacity * 4 / 1024, self.rw_cache_cold.capacity  * 4 / 1024)
    ...
    def read_ext(self, extent): #读操作
        ...
        if self.read_bytes_2 > self.pre_read_bytes_2 + self.change_interval:
            self.auto_change_size()
            self.pre_read_bytes_2 = self.read_bytes_2
    ...
    def write_ext(self, extent, data=None): #写操作
        ...
        if self.written_bytes_2 > self.pre_written_bytes_2 + self.change_interval:
            self.auto_change_size()
            self.pre_written_bytes_2 = self.written_bytes_2
...
class FlashMetadata(object):
    def __init__(self, confobj, counter): # 疑问： 没有Global Mapping Directory (GMD)? --- 如果不在memory里cache，如何知道对应的存储mapping table的物理位置?  --- gtd
        ...
        self.pb_mapping_table_size = 75 * 1024 * 1024
...
class FrameLogPLR:
    ...
    def __init__(self, conf, metadata, counter, gamma, max_size=1*1024**2, frame_length=256):
        ...
        self.leaftl_hit = 0
        self.leaftl_size = 75*MB # 584056832 bytes = 557 MB
