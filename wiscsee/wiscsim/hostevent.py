...
class Event(HostEventBase):
    def __init__(self, sector_size, pid, operation, offset, size,
        ...
        self.sector = int(math.floor((self.offset / sector_size) % self.page_num ))

        #assert self.size % sector_size == 0, \
          #  "size {} is not multiple of sector size {}".format(
           # self.size, sector_size)

        self.sector_end = int(math.ceil(((self.offset+self.size)/sector_size) % self.page_num))

        self.sector_count = self.sector_end - self.sector + 1
